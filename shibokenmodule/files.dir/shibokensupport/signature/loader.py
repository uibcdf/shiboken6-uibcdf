...
are loaded completely after 'import PySide6_uibcdf'.
...
def move_into_pyside_package():
    ...
    try:
        import PySide6_uibcdf.support
    except ModuleNotFoundError:
        # This can happen in the embedding case.
        put_into_package(PySide6_uibcdf, shibokensupport, "support")
    if not is_pypy:
        put_into_package(PySide6_uibcdf.support, feature)
    put_into_package(PySide6_uibcdf.support, signature)
    put_into_package(PySide6_uibcdf.support.signature, mapping)
    put_into_package(PySide6_uibcdf.support.signature, errorhandler)
    put_into_package(PySide6_uibcdf.support.signature, layout)
    put_into_package(PySide6_uibcdf.support.signature, lib)
    put_into_package(PySide6_uibcdf.support.signature, parser)
    put_into_package(PySide6_uibcdf.support.signature, importhandler)
    put_into_package(PySide6_uibcdf.support.signature.lib, enum_sig)
    put_into_package(PySide6_uibcdf.support.signature.lib, pyi_generator)
    put_into_package(PySide6_uibcdf.support.signature.lib, tool)
...
if "PySide6_uibcdf" in sys.modules:
    # We publish everything under "PySide6_uibcdf.support", again.
    move_into_pyside_package()
    # PYSIDE-1502: Make sure that support can be imported.
    try:
        import PySide6_uibcdf.support
    except ModuleNotFoundError:
        print("PySide6_uibcdf.support could not be imported. "
    ...
    def post_init():
        ...
        if not is_pypy:
            ...
            import PySide6_uibcdf.support.feature
            sys.modules["__feature__"] = PySide6_uibcdf.support.feature
